...
現在の収録数: **338 名 / 258 要素 / 23 性癖 / 26 軸**
...
| ⑤ 識別 | **モデル自身の作品知識** | 目の形、顔、口調（他のどのレーンにも出てこない識別子） | `trait.py` | `trait.json` |
...
- **⑤識別レーンだけは、モデルの作品知識を根拠にしてよい。** ①〜④と `classify.py` は
  「証拠に書かれたことしか採らない」という縛りで動く。誤りを防ぐには正しいが、
  そのぶん構造的に取りこぼす種類の特徴がある。市丸ギンの糸目と関西弁がそれで、
  Danbooru のタグ付与率では閾値に届かず（`closed_eyes` は瞬きと区別されない）、
  AniList の英語あらすじにも書かれない。どのレーンからも永久に出てこないが、
  読者がその人物を思い出すときに真っ先に挙げるものだ。
  代わりに ⑤ には 三つの縛りを置く: 広く共有された認識かを自己申告させる、
  根拠をファンの言い回しで書かせる、`src: knowledge` で他レーンと混ぜない。

### 識別とは何か

**この辞典が集めるのは共通項ではなく識別子。** 「短髪」は 338 名中 129 名が持つので、
どれだけ確実でも誰のことも指さない。`merge.py` の `rarity_demote` が、収録済みの
人物との共有率で比重を機械的に下げる（20% 以上なら spice、8% 以上なら sub どまり）。
境目は実測から引いた——共有率の中央値は 1.5%、上位 1 割の境目が 7.4% で、そこから上は
短髪 38% / 長髪 36% / 黒髪 25% と急に伸びる。

`python3 tools/report.py identity` で、共有率の高い語ばかりで出来ている人物を洗い出せる。
